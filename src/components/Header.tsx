    ...
    <header className="fixed top-0 left-0 right-0 z-50 bg-background/90 backdrop-blur-md border-b border-border">
      <div className="container mx-auto px-6 h-16 flex items-center justify-between">
        <a href="#" className="flex items-center gap-2.5 min-w-0">
          <KGSCoin size={36} className="md:w-10 md:h-10 shrink-0" />
          <div className="flex flex-col leading-none min-w-0">
            <span className="font-display text-sm md:text-base font-bold text-foreground tracking-wide truncate">KGS FLOW</span>
            <span className="text-[8px] md:text-[9px] font-body text-muted-foreground tracking-widest uppercase truncate">Kora Global Systems</span>
          </div>
        </a>
        ...
        <div className="md:hidden bg-background/95 backdrop-blur-md border-b border-border px-6 pb-5 pt-3 space-y-3 animate-in slide-in-from-top-2 duration-200">
          {navItems.map((item) => (
            <button
              key={item.key}
              onClick={() => scrollTo(item.href)}
              className="block w-full text-left text-sm font-body text-muted-foreground hover:text-primary transition-colors py-1.5 border-b border-border/30 last:border-0"
            >
              {t(item.key)}
            </button>
          ))}
          <button
            onClick={() => scrollTo("#contact")}
            className="block w-full text-sm font-medium bg-primary text-primary-foreground px-5 py-2.5 rounded-lg mt-2"
          >
            {t("nav.cta")}
          </button>
        </div>
